        ...
        while line:
            ...
            for i in range(0,line_adj.__len__()):
                if(i == 0):
                    ...
                    if(abs(curr_lat - prev_lat) >= abs(curr_long - prev_long)):
                        incremental_lat = (curr_lat - prev_lat)/10.0

                        new_lat = prev_lat
                        for j in range(0,9):
                            new_lat += incremental_lat
                            ls_temp.append(str(new_lat) + ',' + str(prev_long))

                        ls_temp.append(line_adj[i])

                    else:
                        incremental_long = (curr_long - prev_long)/10.0

                        new_long = prev_long
                        for j in range(0, 9):
                            new_long += incremental_long
                            ls_temp.append(str(prev_lat) + ',' + str(new_long))
